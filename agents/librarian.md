...
At most {{maxTurns}} turns total (including the final answer turn). This is a cap, not a target.
...
- Symbol/text known: start with `gh search code ... --limit {{maxSearchResults}}` (plus `--repo` / `--owner` filters when available).
...
1) Code search:
   `gh search code '<terms>' --json path,repository,sha,url,textMatches --limit {{maxSearchResults}}`
