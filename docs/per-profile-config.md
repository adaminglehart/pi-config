...
```
Base → Environment → Environment Local → Profile → Profile Local
```
...
3. `config/work/mcp.local.json` (machine-specific, gitignored)
4. `profiles/coding/config/mcp.json` (profile-specific override)
5. `profiles/coding/config/mcp.local.json` (machine-specific profile override, gitignored)

The `*.local.json` files are gitignored and useful for machine-specific tools (e.g., internal MCP servers, local paths) that shouldn't be committed.
...
For machine-specific config that shouldn't be committed (internal tools, local paths, etc.), use `*.local.json` files. These follow the same structure but are gitignored:

```bash
# Add an MCP server only on this machine
cat > config/work/mcp.local.json <<EOF
{
  "mcpServers": {
    "internal-tool": {
      "command": "/path/to/internal-mcp-server"
    }
  }
}
EOF
...
Local files are merged after their non-local counterpart, so they can override or extend any tracked config.

## Environment Variables in Config

JSON config values can reference environment variables with `${VAR_NAME}` syntax. These are resolved at build time from your `.env` file (loaded by mise):
...
```bash
MY_LLM_BASE_URL=https://my-proxy.example.com/v1
MY_LLM_API_KEY=my-secret-key
```

## Supported Config Files

Any config file with a `config/*.base.json` can have environment, local, and profile overrides:
...
4. For machine-specific values, use `*.local.json` at any layer
