...
function extractEventAssistantText(message: SessionEventMessage): string {
...
export default function (pi: ExtensionAPI) {
	...
	type ToolArgumentValue = string | number | boolean | null | object;
	type ToolArguments = Record<string, ToolArgumentValue>;

	function formatToolArgs(toolName: string, args: ToolArguments): string {
		switch (toolName) {
			case "bash":
				return typeof args.command === "string" ? truncateToWidth(args.command.split("\n")[0], 50, "…") : "";
			case "read":
			case "write":
			case "edit":
		...
		const unsubscribe = session.subscribe((event: AgentSessionEvent) => {
			...
			switch (event.type) {
				...
				case "tool_execution_start": {
					...
					try {
						pendingToolCalls.push({
							...
							toolName,
							args: formatToolArgs(toolName, event.args as ToolArguments),
							status: "running",
						});
					} catch {
				...
				case "tool_execution_end": {
					const endToolName = event.toolName;
