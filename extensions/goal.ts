...
function normalizeStatus(value: JsonValue | undefined): GoalStatus {
	switch (value) {
		case "active":
		case "paused":
		case "blocked":
...
function normalizeNonNegativeInteger(value: JsonValue | undefined, fallback = 0): number {
	if (typeof value !== "number" || !Number.isFinite(value)) return fallback;
	return Math.max(0, Math.floor(value));
}

function normalizeGoal(value: JsonValue | undefined): Goal | null {
...
function assistantUsageTokens(messages: AgentEndEvent["messages"]): number {
	let total = 0;
	for (const message of messages) {
		...
		total += measured > 0 ? measured : Math.max(0, message.usage.totalTokens ?? 0);
	}
	return total;
}
...
export default function goalExtension(pi: ExtensionAPI) {
	...
	function reconstructState(ctx: ExtensionContext): void {
		...
		for (const entry of ctx.sessionManager.getBranch()) {
			...
			const data = entry.data as { goal?: JsonValue } | undefined;
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		if (activeGoalIdAtAgentStart === goal.id) {
			const tokens = assistantUsageTokens(event.messages);
			if (tokens > 0) {
