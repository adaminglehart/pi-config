// Ported from mitsuhiko/agent-stuff at commit 13bc8f87970bec8830aab0f1c0487d35aa7c0917.
// Modified for this pi-config repository and its pinned Pi APIs.
// Licensed under Apache-2.0; repository copy: licenses/agent-stuff-Apache-2.0.txt.
// Upstream license: https://github.com/mitsuhiko/agent-stuff/blob/13bc8f87970bec8830aab0f1c0487d35aa7c0917/LICENSE
...
class ProgressBorderedLoader extends Container {
	private readonly loader: CancellableLoader;

	constructor(tui: TUI, theme: ExtensionContext["ui"]["theme"], message: string) {
		super();
...
function isFauxModelReference(parts: ModelReferenceParts): boolean {
...
interface ProviderModelAndUsage extends ModelReferenceParts {
	usage?: JsonValue;
}

function extractProviderModelAndUsage(obj: JsonObject): ProviderModelAndUsage {
	// Session format varies across versions.
	// - Newer: { provider, model, usage } on the message wrapper
	// - Older: { message: { provider, model, usage } }
	const message = isJsonObject(obj.message) ? obj.message : undefined;
	return {
		api: obj.api ?? message?.api,
...
function readJsonNumber(value: JsonValue | undefined): number {
...
function extractCostTotal(usage: JsonValue | undefined): number {
	...
	const directCost = readJsonNumber(cost);
	if (directCost > 0) return directCost;
	return isJsonObject(cost) ? readJsonNumber(cost.total) : 0;
}

function extractTokensTotal(usage: JsonValue | undefined): number {
	// Usage format varies across providers and pi versions.
	// We try direct totals, a nested tokens object, and sums of input/output parts.
	if (!isJsonObject(usage)) return 0;

	let total =
		readJsonNumber(usage.totalTokens) ||
		readJsonNumber(usage.total_tokens) ||
		readJsonNumber(usage.tokens) ||
		readJsonNumber(usage.tokenCount) ||
		readJsonNumber(usage.token_count);
	if (total > 0) return total;

	const tokens = isJsonObject(usage.tokens) ? usage.tokens : undefined;
	total =
		readJsonNumber(tokens?.total) ||
		readJsonNumber(tokens?.totalTokens) ||
		readJsonNumber(tokens?.total_tokens);
	if (total > 0) return total;

	const input =
		readJsonNumber(usage.promptTokens) ||
		readJsonNumber(usage.prompt_tokens) ||
		readJsonNumber(usage.inputTokens) ||
		readJsonNumber(usage.input_tokens);
	const output =
		readJsonNumber(usage.completionTokens) ||
		readJsonNumber(usage.completion_tokens) ||
		readJsonNumber(usage.outputTokens) ||
		readJsonNumber(usage.output_tokens);
...
export default function sessionBreakdownExtension(pi: ExtensionAPI) {
	pi.registerCommand("session-breakdown", {
		...
		handler: async (_args, ctx: ExtensionContext) => {
			if (ctx.mode !== "tui") {
				// Non-interactive fallback: send a compact summary.
			...
			const data = await ctx.ui.custom<BreakdownData | null>((tui, theme, _kb, done) => {
				...
				const loader = new ProgressBorderedLoader(tui, theme, baseMessage);
				...
				loader.setMessage(renderMessage());
