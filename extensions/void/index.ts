...
const NodeNames = ["†-I", "†-II", "☽-A", "☽-B", "★-01", "★-02", "◈-i", "◉-i"] as const;

// ═════════════════════════════════════════════════════════════════════════════
// Types
// ═════════════════════════════════════════════════════════════════════════════
type EntityMood = "idle" | "thinking" | "happy" | "sleep" | "excited" | "coding";
...
const _px24 = (k: keyof typeof Colors) => _fg24(Colors[k][0], Colors[k][1], Colors[k][2]);
...
class Entity {
	...
	constructor() {
		this.state = { xp: 0, totalTurns: 0, born: new Date().toISOString(), totalTokens: 0 };
...
class ParticleField {
	...
	spawn(tokensThisTurn: number, isWorking: boolean) {
		...
		for (let i = 0; i < spawnCount && this.particles.length < this.MAX_PARTICLES; i++) {
			...
			const life = isWorking ? 0.3 + Math.random() * 0.5 : 0.8 + Math.random() * 1.2;
	...
	render(cols: number, rows: number): string[] {
		...
		for (let r = 0; r < rows; r++) {
			...
			for (let c = 0; c < cols; c++) {
				...
				const charIdx = Math.min(this.DREAM_CHARS.length - 1, Math.round(boosted * (this.DREAM_CHARS.length - 1)));
				const ch = this.DREAM_CHARS[charIdx];

				const warmth = 0.4 + Math.min(1.3, boosted) * 0.6 + this.activityLevel * 0.1;
...
function renderVoid(
	...
	// ── Top border ──
	lines.push(borderColor + S.frame.tl + S.frame.h.repeat(innerW) + S.frame.tr + _rst);
	...
	const xpColorStart = xpFilled >= barW * 0.8 ? kurenai : xpFilled >= barW * 0.5 ? kin : _px24("bara");
	const xpBar = xpColorStart + S.selected.repeat(xpFilled) + _rst + hai + S.unselected.repeat(barW - xpFilled) + _rst;

	const fmtK = (n: number) => n >= 1_000_000 ? `${(n / 1_000_000).toFixed(1)}M` : n >= 1000 ? `${(n / 1000).toFixed(0)}k` : `${n}`;
	const tokenStr = sessionTokens.total > 0 ? sumi + fmtK(sessionTokens.total) + _rst : "";

	const row1Content = kurenai + nameTag + _rst + tsuki + levelTag + _rst + kin + stageTag + _rst + " " + xpBar + " " + tokenStr;
	const row1Padded = padContent(row1Content, innerW);
	lines.push(borderColor + S.frame.v + _rst + row1Padded + borderColor + S.frame.v + _rst);

	// ── Row 2: Face + spinner + status ──
	const spinnerPhases = ["◐", "◑", "●", "◑", "◐", "○"];
	const spinner = isWorking ? rosoku + spinnerPhases[workingFrame % spinnerPhases.length] + _rst : "";
	const face = entity.getFrame();
	const statusLine = entity.getStatusLine();
	const statusWidth = innerW - 15;

	const row2Content = kurenai + face + _rst + " " + spinner + (spinner ? " " : "") + tsuki + softMarquee(statusLine, Math.max(5, statusWidth), marqueeTick, 8, 10) + _rst;
	const row2Padded = padContent(row2Content, innerW);
	lines.push(borderColor + S.frame.v + _rst + row2Padded + borderColor + S.frame.v + _rst);
	...
	lines.push(borderColor + S.frame.v + _rst + pLinePadded + borderColor + S.frame.v + _rst);

	// ── Bottom border ──
	lines.push(borderColor + S.frame.bl + S.frame.h.repeat(innerW) + S.frame.br + _rst);
...
export default function voidExtension(pi: ExtensionAPI) {
	...
	function render() {
		...
		const lines = renderVoid(entity, particles, width, marqueeTick, workingFrame, isWorking, lastTokens);
