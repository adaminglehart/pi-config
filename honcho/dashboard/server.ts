...
async function listMessages(ws: string, sid: string, _page = 1, _size = 50) {
  const honcho = new Honcho({ baseURL: API, apiKey: "none", workspaceId: ws });
  const session = await honcho.session(sid);
  // SDK's messages() takes filters (not pagination params) - returns all messages
...
function navLinks(ws?: string, active?: string): string {
  const links = [
    `<a href="/" class="${active === "home" ? "active" : ""}">Workspaces</a>`,
  ];
  if (ws) {
    links.push(
      `<a href="/w/${esc(ws)}" class="${active === "overview" ? "active" : ""}">Overview</a>`,
    );
    links.push(
      `<a href="/w/${esc(ws)}/peers" class="${active === "peers" ? "active" : ""}">Peers</a>`,
    );
    links.push(
      `<a href="/w/${esc(ws)}/sessions" class="${active === "sessions" ? "active" : ""}">Sessions</a>`,
    );
    links.push(
      `<a href="/w/${esc(ws)}/conclusions" class="${active === "conclusions" ? "active" : ""}">Conclusions</a>`,
    );
    links.push(
      `<a href="/w/${esc(ws)}/queue" class="${active === "queue" ? "active" : ""}">Queue</a>`,
    );
  }
  return links.join("");
}
...
async function handleHome(): Promise<string> {
  ...
  for (const ws of items) {
    ...
    rows += `<tr>
      ...
      <td class="meta">${timeAgo(ws.createdAt)}</td>
    </tr>`;
  }
  return layout(
    "Workspaces",
    navLinks(undefined, "home"),
    `
    <h1>Workspaces</h1>
...
async function handleWorkspace(ws: string): Promise<string> {
  ...
  const pct = queue
    ? Math.round(
        (queue.completedWorkUnits / Math.max(queue.totalWorkUnits, 1)) *
          100,
      )
    : 0;
  ...
  return layout(
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;margin-bottom:4px">
        <span>${queue.completedWorkUnits} / ${queue.totalWorkUnits} work units</span>
        <span class="badge ${pct === 100 ? "badge-green" : "badge-orange"}">${pct}%</span>
      </div>
      <div class="progress"><div class="progress-bar" style="width:${pct}%"></div></div>
      <div class="meta" style="margin-top:8px">Pending: ${queue.pendingWorkUnits} · In progress: ${queue.inProgressWorkUnits}</div>
    </div>`
        : ""
    }
...
async function handlePeers(ws: string): Promise<string> {
  ...
  for (const peer of items) {
    ...
    html += `<div class="card">
      ...
      <div class="meta"><a href="/w/${esc(ws)}/peers/${esc(peer.id)}">${sessions.total} session${sessions.total !== 1 ? "s" : ""}</a></div>
      ${card?.peer_card ? `<div class="content-full">${esc(card.peer_card)}</div>` : ""}
      ${peer.metadata && Object.keys(peer.metadata).length > 0 ? `<div class="meta" style="margin-top:8px">Metadata: ${esc(JSON.stringify(peer.metadata))}</div>` : ""}
      ${peer.configuration && Object.keys(peer.configuration).length > 0 ? `<div class="meta">Config: ${esc(JSON.stringify(peer.configuration))}</div>` : ""}
    </div>`;
  }
  return layout(
    `Peers — ${ws}`,
...
async function handlePeer(ws: string, peerId: string): Promise<string> {
  ...
  const aboutPeer = allConclusions.items.filter(
    (c) => c.observed_id === peerId,
  );
    ...
    for (const c of aboutPeer) {
      conclusionHtml += `<div class="card">
        <div style="display:flex;justify-content:space-between;align-items:start">
          ...
          <div class="meta" style="margin-left:12px;white-space:nowrap">by <a href="/w/${esc(ws)}/peers/${esc(c.observer_id)}"><span class="badge badge-blue">${esc(c.observer_id)}</span></a><br>${timeAgo(c.createdAt)}</div>
        </div>
      </div>`;
    }
  }
  if (byPeer.length > 0) {
    ...
    for (const c of byPeer.slice(0, 20)) {
      conclusionHtml += `<div class="card">
        <div style="display:flex;justify-content:space-between;align-items:start">
          <div style="flex:1;white-space:pre-wrap;font-size:13px">${esc(c.content)}</div>
          <div class="meta" style="margin-left:12px;white-space:nowrap">about <a href="/w/${esc(ws)}/peers/${esc(c.observed_id)}"><span class="badge badge-green">${esc(c.observed_id)}</span></a><br>${timeAgo(c.createdAt)}</div>
        </div>
      </div>`;
    }
    if (byPeer.length > 20) {
...
async function handleSession(ws: string, sid: string): Promise<string> {
  ...
  for (const m of msgs.items) {
    ...
    msgHtml += `<tr>
      <td><a href="/w/${esc(ws)}/peers/${esc(m.peerId)}"><span class="badge badge-blue">${esc(m.peerId)}</span></a></td>
      <td style="max-width:600px;white-space:pre-wrap;font-size:13px">${esc(content)}</td>
      <td class="meta">${m.tokenCount}</td>
      <td class="meta">${timeAgo(m.createdAt)}</td>
    </tr>`;
  }

  let summaryHtml = "";
  if (summaries?.shortSummary) {
    summaryHtml += `<div class="card"><h3>Short Summary <span class="meta">(${summaries.shortSummary.tokenCount} tokens)</span></h3><div class="content-full">${esc(summaries.shortSummary.content)}</div></div>`;
  }
  if (summaries?.longSummary) {
    summaryHtml += `<div class="card"><h3>Long Summary <span class="meta">(${summaries.longSummary.tokenCount} tokens)</span></h3><div class="content-full">${esc(summaries.longSummary.content)}</div></div>`;
  }
...
async function handleConclusions(ws: string, page: number): Promise<string> {
  ...
  for (const c of data.items) {
    rows += `<tr>
      ...
      <td class="meta">${timeAgo(c.createdAt)}</td>
    </tr>`;
  }
    ...
    for (let i = 1; i <= data.pages; i++) {
      links.push(
        i === page
          ...
          : `<a href="/w/${esc(ws)}/conclusions?page=${i}">${i}</a>`,
      );
    }
...
async function handleQueue(ws: string): Promise<string> {
  ...
  if (!queue)
    return layout(
      `Queue — ${ws}`,
      navLinks(ws, "queue"),
      `<div class="empty">Queue status unavailable</div>`,
    );

  const pct = Math.round(
    (queue.completedWorkUnits / Math.max(queue.totalWorkUnits, 1)) * 100,
  );
  let sessionRows = "";
  for (const [, s] of Object.entries(queue.sessions || {})) {
    if (!s) continue;
    const sPct = Math.round(
      (s.completedWorkUnits / Math.max(s.totalWorkUnits, 1)) * 100,
    );
    sessionRows += `<tr>
      <td><a href="/w/${esc(ws)}/sessions/${esc(s.sessionId)}">${esc(s.sessionId)}</a></td>
      <td>${s.completedWorkUnits} / ${s.totalWorkUnits}</td>
      <td>${s.pendingWorkUnits}</td>
      <td>${s.inProgressWorkUnits}</td>
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    try {
      // /
      if (path === "/" || path === "") {
        return new Response(await handleHome(), {
      ...
      if (wsMatch) {
        return new Response(
          await handleWorkspace(decodeURIComponent(wsMatch[1])),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      if (peerMatch) {
        return new Response(
          await handlePeer(
            decodeURIComponent(peerMatch[1]),
            decodeURIComponent(peerMatch[2]),
          ),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      if (peersMatch) {
        return new Response(
          await handlePeers(decodeURIComponent(peersMatch[1])),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      if (sessionsMatch) {
        return new Response(
          await handleSessions(decodeURIComponent(sessionsMatch[1])),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      if (conclusionsMatch) {
        ...
        return new Response(
          await handleConclusions(
            decodeURIComponent(conclusionsMatch[1]),
            page,
          ),
          { headers: { "Content-Type": "text/html" } },
        );
      ...
      if (queueMatch) {
        return new Response(
          await handleQueue(decodeURIComponent(queueMatch[1])),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      return new Response(`<h1>Error</h1><pre>${esc(msg)}</pre>`, {
        status: 500,
        headers: { "Content-Type": "text/html" },
      });
    }
